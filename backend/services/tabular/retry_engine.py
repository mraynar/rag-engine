...
from backend.services.tabular.executor import execute_query
...
class ColumnRetry(RetryStrategyBase):
    ...
    def apply(self, plan: QueryPlan, resolved: ResolvedEntities, schema: dict) -> Optional[QueryPlan]:
        if plan.aggregation and plan.aggregation.column:
            ...
            from backend.services.tabular.registries import COLUMN_ALIASES
...
class OperatorRetry(RetryStrategyBase):
    ...
    def apply(self, plan: QueryPlan, resolved: ResolvedEntities, schema: dict) -> Optional[QueryPlan]:
        new_filters = []
        modified = False
        from backend.services.tabular.registries import OPERATORS
...
def execute_with_retry(
    ...
    """
    from backend.services.tabular.registries import get_schema
